 ...
 */
import { describe, it, _expect, _vi } from 'vitest'
...
const projectArbitrary = fc.record({
  ...
  tags: fc.array(fc.stringMatching(/^[A-Za-z][A-Za-z0-9.]{0,15}$/), {
    minLength: 1,
    maxLength: 5,
  }),
  github: fc.option(fc.constant('https://github.com/test/repo'), {
    nil: undefined,
  }),
...
describe('ProjectCard - Project Information Completeness Property Tests', () => {
  ...
  it('Property 2: Project card displays project name', () => {
    fc.assert(
      fc.property(projectArbitrary, project => {
        ...
        const hasName =
          nameElement.exists() &&
          nameElement.text().trim() === project.name.trim()
    ...
    fc.assert(
      fc.property(projectArbitrary, project => {
        ...
        const hasDescription =
          descElement.exists() &&
          descElement.text().trim() === project.description.trim()
    ...
    fc.assert(
      fc.property(
        ...
        project => {
          ...
          const hasGithubLink = links.some(
            link =>
    ...
    fc.assert(
      fc.property(projectArbitrary, project => {
        ...
        const hasAccessibleLabel =
          ariaLabelledBy !== undefined && ariaLabelledBy.length > 0
  ...
  it('Property 2: Project tags have list role for accessibility', () => {
    fc.assert(
      fc.property(projectArbitrary, project => {
        ...
        const tags = wrapper.findAll('.tag')
        const allTagsHaveListItemRole = tags.every(
          tag => tag.attributes('role') === 'listitem'
        )
  ...
  it('Property 2: External links have proper security attributes', () => {
    fc.assert(
      fc.property(
        projectArbitrary.filter(
          p => p.github !== undefined || p.demo !== undefined
        ),
        project => {
          const wrapper = mount(ProjectCard, {
            props: { project },
