...
const timelineTypeArb = fc.constantFrom(
  'work',
  'education',
  'community',
  'competition'
...
const timelineItemsArb = fc.array(timelineItemArb, {
  minLength: 1,
  maxLength: 20,
})
...
describe('FilterableTimeline - Property 2: 時間軸篩選正確性', () => {
  ...
  it('Property 2: Filtering by type shows only items of that type', () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...filterTypes), filterType => {
  ...
  it('Property 2: Filtered items count matches expected count for each type', async () => {
    ...
    for (const filterType of filterTypes) {
      ...
      const expectedCount = sampleTimelineItems.filter(
...
describe('FilterableTimeline - Property 2: 時間軸篩選正確性 (Generated Data)', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (items, filterType) => {
          ...
          const filteredItems = wrapper.vm.filteredItems
          const allMatchType = filteredItems.every(
            item => item.type === filterType
          )
  ...
  it('Property 2: For any generated timeline items, "all" filter returns all items', () => {
    fc.assert(
      fc.property(timelineItemsArb, items => {
        const itemsWithIds = items.map((item, index) => ({
          ...item,
          id: `item-${index}`,
          expanded: false,
...
describe('FilterableTimeline - Property 3: 時間軸展開功能', () => {
  ...
  beforeEach(() => {
    ...
    const freshItems = sampleTimelineItems.map(item => ({
      ...item,
      expanded: false,
    }))
    wrapper = mount(FilterableTimeline, {
    ...
    fc.assert(
      fc.property(
        ...
        itemIndex => {
          ...
          return (
            afterToggle === !initialState && afterSecondToggle === initialState
          )
        }
      ),
      { numRuns: 100 }
