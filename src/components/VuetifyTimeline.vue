          ...
          <template v-if="!compact">
            <v-card class="timeline-card" elevation="2">
              <v-card-title class="timeline-title">
                <v-icon class="timeline-icon mr-2"
                  ><svg viewBox="0 0 24 24" style="width: 1em; height: 1em">
                    <path :d="item.icon" fill="currentColor" /></svg
                ></v-icon>
                {{ item.title }}
              </v-card-title>
          ...
          <template v-else>
            <div class="timeline-card-compact">
              <div class="timeline-title-compact">
                <span class="timeline-icon-compact"
                  ><svg viewBox="0 0 24 24" style="width: 1em; height: 1em">
                    <path :d="item.icon" fill="currentColor" /></svg
                ></span>
                <span>{{ item.title }}</span>
              </div>
              <div v-if="item.company" class="timeline-company-compact">
                {{ item.company }}
              </div>
              <div v-if="item.duration" class="timeline-duration-compact">
                {{ item.duration }}
              </div>
              <div v-if="item.description" class="timeline-description-compact">
                {{ item.description }}
              </div>
